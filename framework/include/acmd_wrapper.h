...
struct ACMD {
    ...
    const char* category;
    const char* kind;
    void (*acmd_function)(ACMD*);

    ACMD();
    ACMD(const char* category, const char* kind, const char* motion_kind, const char* acmd_name, void (*acmd_func)(ACMD*));

    bool match();
    void run(lib::L2CAgent*);
    void nullify_original(lib::L2CAgent*);

    void _frame(float f);
    // attempted reimplementation of sv_animcmd::frame
    bool _frame_reimpl(float f);
    void _wait(float f);
